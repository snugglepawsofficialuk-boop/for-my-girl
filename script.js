...
(function music(){
  ...
  /* if the browser blocks a play attempt, glow the pill instead of hijacking
     her next tap — a deliberate tap on the pill always carries the gesture */
  const nudge = () => {
  ...
  const useMp3 = () => {
    ...
    musicPlay  = () => audio.play().catch(() => nudge());
  ...
  if (hasYT){
    ...
    window.onYouTubeIframeAPIReady = () => {
      ytPlayer = new YT.Player("ytHolder", {
        ...
        events: {
          ...
          onError: () => { if (hasMp3) useMp3(); nudge(); }
        }
      });
    };
    ...
    musicPlay = () => {
      if (!ytReady){ wantPlay = true; nudge(); return; }
      try {
...
(function ambient(){
  ...
  let w, h, dpr, petals = [], dust = [], orbs = [], shooters = [], nextShoot = 0, raf = null;
  ...
  const frame = (t) => {
    ...
    for (const o of orbs){
      ...
      const c = o.gold ? "217,164,65" : "200,40,70";
